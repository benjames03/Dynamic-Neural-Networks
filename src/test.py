...
def eval_model():
    ...
    for state in [False, True]:
        ...
        if state: print("\r", end="")
        test_loss /= len(loader)
        correct /= len(loader.dataset)
        stop = time.time()
        print(f"Simulating: {state}, Accuracy: {correct}, Loss: {test_loss} ({stop-go:.2f}s)")
    
def eval_conv_layer():
    start = time()
    input_cube = torch.rand((1, 30, 32, 32))
    simulated = conv.DirectConv2d(in_channels=30, out_channels=13, kernel_size=3, stride=1)
    benchmark = nn.Conv2d(in_channels=30, out_channels=13, kernel_size=3, stride=1)
