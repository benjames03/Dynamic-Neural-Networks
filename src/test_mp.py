...
def eval_conv_layer():
    input_cube = torch.rand((2, 3, 32, 32))
...
if __name__ == "__main__":
    ...
    accuracy = 0
    for processes in results.keys():
        start = time.time()
        accuracy += eval_model_mp(processes)
        end = time.time()
        results[processes] = (end-start)*10/processes
    print(results)
